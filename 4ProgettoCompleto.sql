...
BEGIN
    ...
    SET @casaautomobilistica = (SELECT CasaAutomobilistica
								FROM Caratteristiche
                                WHERE Targa = NEW.TargaVeicoloProponente);
...
CREATE TRIGGER aggiungi_voto_ride_sharing_fruitore #aggiunge automaticamente i voti dei ride sharing
...
/*PROBLEMA: Quando aggiungo automaticamente le stelle potrei avere problemi con chiavi AUTO_INCREMENT dello stesso valore, come posso fare?
1) Sommo a ciascun valore di CodVoto un numero fissato a priori, ma mi devo ricordare quale è per riprenderlo. ES. Quando inserisco su StelleUtente un voto di StelleProponenteRideSharing faccio CodVoto = CodVoto+1000. Così ho comunque valore diversi ma riconducibili a quelli precedenti.
2) Faccio una nuova Primary key per StelleUtente. */    
    
    
/*CREATE TRIGGER aggiungi_stelle_ride_sharing #aggiunge automaticamente le stelle dei ride sharing
...
BEGIN
	INSERT ValutazioneUtente
    SET CodVoto = NEW.CodVoto+2000, Id = NEW.IdProponente, Ruolo = 'Fruitore', Recensione = NEW.Recensione;
END $$ 

/*CREATE TRIGGER aggiungi_stelle_proponente_noleggio #aggiunge automaticamente le stelle dei ride sharing
