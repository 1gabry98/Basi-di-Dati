...
BEGIN
   ...
   SET _targa = (SELECT Targa
    ...
    SET Targa = _targa, CodStrada = NEW.CodStrada,CodSharing = NEW.CodSharing, Password_ = 'root', kmStrada = NEW.numChilometro, Timestamp_ = current_timestamp();
    
END $$

CREATE PROCEDURE aggiorna_tracking (IN _codSharing INT)
BEGIN
	SET @orariostop = (SELECT OraStimatoArrivo
						FROM RideSharing
						WHERE CodSharing = _codSharing);
	
    SET @orarioinizio = (SELECT Orapartenza
						FROM RideSharing
						WHERE CodSharing = _codSharing);
                        
	SELECT @orarioinizio, @orariostop;
...
BEGIN
    ...
    DECLARE stima TIMESTAMP DEFAULT '0000-00-00 00:00:00';
...
END $$
 ...
 -- soluzione grezza per il calcolo dell'orario di arrivo stimato
 CREATE PROCEDURE OrarioArrivoGrezzo(IN _codSharing INT)
 BEGIN
	DECLARE _limite DOUBLE DEFAULT 0;
    DECLARE _km INT DEFAULT 0;
    DECLARE tempo DOUBLE DEFAULT 0;
    DECLARE minuti DOUBLE DEFAULT 0;
    DECLARE ore DOUBLE DEFAULT 0;
	DECLARE orastimata DOUBLE DEFAULT 0;
    DECLARE minutostimato DOUBLE DEFAULT 0;
    
	SET _limite = (SELECT AVG(LDV.ValoreLimite)
					FROM LimitiDiVelocita LDV
						INNER JOIN StradeTragittoSharing STS ON LDV.CodStrada = STS.CodStrada
					WHERE STS.CodSharing = _CodSharing
                    ORDER BY STS.CodStrada);
	
    SET _km = (SELECT kmPercorsi
				FROM TragittoSharing 
                WHERE CodTragitto = _codSharing);
	
    
	SET tempo = (_km/_limite)*60;
    ...
    UPDATE RideSharing
    SET OraStimatoArrivo = OraPartenza + INTERVAL tempo MINUTE;
...
-- RIDE SHARING
INSERT INTO `progetto`.`ridesharing` (`IdProponente`, `IdFruitore`, `OraPartenza`, `Targa`) VALUES ('1', '2', '2018-12-17 08:22:00', 'AE987CB');
...
INSERT INTO `progetto`.`ridesharing` (`IdProponente`, `IdFruitore`, `OraPartenza`, `Targa`) VALUES ( '3', '4', '2018-12-17 09:00:00', 'AW123OB');
INSERT INTO `progetto`.`adesioniridesharing` (`CodSharing`, `IdUtente`) VALUES ('2', '4');
...
INSERT INTO `progetto`.`stradetragittosharing` (`CodStrada`, `CodSharing`, `kmInizioStrada`, `kmFineStrada`) VALUES ('1', '2', '10', '40');
INSERT INTO `progetto`.`stradetragittosharing` (`CodStrada`, `CodSharing`, `kmInizioStrada`, `kmFineStrada`) VALUES ('2', '2', '5', '20');

-- RIDE SHARING
INSERT INTO TragittoSharing (kmPercorsi) VALUES ('60');
INSERT INTO TragittoSharing (kmPercorsi) VALUES ('60');
-- POSIZIONE SHARING
INSERT INTO PosizionePartenzaSharing VALUES ('1','1', '1');
INSERT INTO PosizioneArrivoSharing VALUES ('1','2', '20');

INSERT INTO PosizionePartenzaSharing VALUES ('2','1', '10');
INSERT INTO PosizioneArrivoSharing VALUES ('2','2', '30');
...
CALL aggiorna_tracking(1);
CALL aggiorna_tracking(2);


CREATE EVENT aggiornamento
ON SCHEDULE EVERY 5 MINUTE STARTS @orarioinizio ENDS @orariostop
DO BEGIN
