...
CREATE TRIGGER consumo_carburante
AFTER INSERT ON SommaCostiAttualePool FOR EACH ROW

BEGIN
	DECLARE _statoiniziale
	

CREATE TRIGGER controllo_condizioni_iniziali
AFTER UPDATE ON PrenotazioneDiNoleggio FOR EACH ROW

BEGIN
	DECLARE carburanteStatoIniziale VARCHAR(7);
	
	IF(NEW.Stato = 'CHIUSO') THEN
		BEGIN
			SET carburanteStatoIniziale = (SELECT QuantitaCarburante
											FROM StatoIniziale
											WHERE Targa = NEW.Targa);
			IF(carburanteStatoIniziale <> 
...
-- SINISTR0 NOLEGGIO
INSERT INTO `progetto`.`sinistronoleggio` ( `TargaVeicoloProponente`, `Orario`, `CodStrada`, `KmStrada`, `Dinamica`, `PercentualeDiResponsabilita`) VALUES ('AE987CB', '2017-01-10 10:31:33', '1', '5', 'Na botta assurda', '10');
...
-- POOL
INSERT INTO `progetto`.`pool` (`GiornoPartenza`, `GiornoArrivo`, `IdProponente`, `Targa`, `GradoFlessibilitA`, `NumPosti`) VALUES ('2018-12-11 14:00:00', '2018-12-11 15:00:00', '3', 'AW123OB', 'BASSO', '4');
