...
BEGIN
    ...
    SET postidisponibili = (SELECT NumPosti
							 FROM AdesioniRideSharing ARS NATURAL JOIN RideSharing RS
								NATURAL JOIN Auto A NATURAL JOIN Caratteristiche C
							 WHERE NEW.CodSharing = RS.CodSharing) - 1;

	SET postioccupati = (SELECT COUNT(IdUtente)
						FROM AdesioniRideSharing
                        WHERE NEW.CodSharing = CodSharing) ;
                        
	SET postirimanenti = postidisponibili - postioccupati - 1; #2 = 1 posto per conducente + 1 nuovo passeggero
    
	IF( postirimanenti < 0) THEN
		SIGNAL SQLSTATE '45000'
...
CREATE TRIGGER aggiorna_posti_disponibile_pool
...
INSERT INTO `progetto`.`adesioniridesharing` (`CodSharing`, `IdUtente`) VALUES ('1', '2');
